...
1. The URL must have a release on GitHub or GitLab so that R-universe can process the package without an error. The package maintainers may either choose to either create their own releases (recommended) or submit the package to CRAN and use the releases from the CRAN mirror at https://github.com/cran.
